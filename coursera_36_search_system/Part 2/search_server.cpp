...
const vector<countId>& SearchServer::ServerLookup(const string_view word)
{
    shared_lock sLock(lockM);
...
void SearchServer::AddQueriesStream(istream& query_input, ostream& search_results_output)
{
    ...
    pool.push(
        async(launch::async, &SearchServer::AddQueriesStreamWorker, this, ref(query_input),
            ref(search_results_output)));
}

void SearchServer::AddQueriesStreamWorker(istream& query_input, ostream& search_results_output)
{
    array<pair<size_t, size_t>, 12000> search_results { make_pair(0, 0) };
    ...
    for (string current_query; getline(query_input, current_query);) {
        ...
        for (const string_view& word : words) {
            for (const auto& docid : ServerLookup(word)) {
